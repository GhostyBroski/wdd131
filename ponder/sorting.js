...
const hikes = [
 ...
 let simpleSortt = simpleList.sort();
 console.log(simpleSortt);
...
let lowerCaseList = filterList.map(item => item.toLowerCase());
console.log(lowerCaseList);

// Hiking Stuff (Similar to Recipes Pt. 2) -----------------------------------------------------
...
let random = Math.floor(Math.random() * hikes.length);  // COULD BE A DICE FUNCTION
console.log(random);
...
function hikeTemplate(hike) { // Just putting in the variables
