...
function generateStars(rating) {
  ...
  return `
    <span class="rating" role="img" aria-label="Rating: ${rating} out of 5 stars">
      ${'<span aria-hidden="true" class="icon-star">⭐</span>'.repeat(full)}
      ${half ? '<span aria-hidden="true" class="icon-star">⯨</span>' : ''}
      ${'<span aria-hidden="true" class="icon-star-empty">☆</span>'.repeat(empty)}
    </span>
  `;
}
